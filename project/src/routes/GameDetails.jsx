...
export default function GameDetails() {
    ...
    const badge = (text) => (
        <span style={{
            display:'inline-block', padding:'4px 8px', border:'1px solid #eee',
            borderRadius:999, background:'#f8f8f8', fontSize:12
        }}>{text}</span>
    )
        ...
        <article>
            <div style={{display:'flex', alignItems:'center', gap:12, marginBottom:12}}>
                <button onClick={() => navigate(-1)}
                        style={{padding:'8px 12px', border:'1px solid #ddd', borderRadius:10, background:'#fff'}}>← Back</button>
                <Link to="/" style={{padding:'8px 12px', border:'1px solid #ddd', borderRadius:10, background:'#fff'}}>Home</Link>
            </div>

            <div style={{
                display:'grid', gridTemplateColumns:'minmax(260px, 320px) 1fr', gap:20, alignItems:'start',
                border:'1px solid #eee', borderRadius:16, overflow:'hidden', boxShadow:'0 6px 18px rgba(0,0,0,.06)'
            }}>
                    ...
                    <div style={{display:'flex', gap:10, flexWrap:'wrap'}}>
                        {game.game_url && (
                            <a href={game.game_url} target="_blank" rel="noreferrer"
                               style={{padding:'10px 14px', background:'#111', color:'#fff', borderRadius:10, textDecoration:'none'}}>
                                Official site
                            </a>
                        )}
